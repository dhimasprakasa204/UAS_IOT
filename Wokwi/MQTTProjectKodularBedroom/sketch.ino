...
void mqttReconnect()
{
  while (!client.connected())
  {
    ...
    if (client.connect("Bedroom","kostgiofamily","LdvL4nYu1abxYNfJ"))
    {
      Serial.println("Connected");
      client.subscribe("henjo/bedroom/led");
    }
    else
    {
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "henjo/bedroom/led") {
...
void kelembaban() {
   ...
   Serial.println(msg1);
   client.publish("henjo/bedroom/kelembaban", msg1); // 
}
...
void temperatur() {
   ...
   snprintf (msg, MSG_DHT, "%s", itoa(nilaiTemperatur,msg,10));
    Serial.println("Suhu");
    Serial.println(msg);
    client.publish("henjo/bedroom/suhu", msg); // 
}