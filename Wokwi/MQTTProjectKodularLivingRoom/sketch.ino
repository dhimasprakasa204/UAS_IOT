...
void mqttReconnect()
{
  while (!client.connected())
  {
    ...
    if (client.connect("LivingRoom","kostgiofamily","LdvL4nYu1abxYNfJ"))
    {
      Serial.println("Connected");
      client.subscribe("henjo/livingroom/led");
    }
    else
    {
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "henjo/livingroom/led") {
...
void kelembaban() {
   ...
   Serial.println(msg1);
   client.publish("henjo/livingroom/kelembaban", msg1); // 
}
...
void temperatur() {
   ...
   snprintf (msg, MSG_DHT, "%s", itoa(nilaiTemperatur,msg,10));
    Serial.println("Suhu");
    Serial.println(msg);
    client.publish("henjo/livingroom/suhu", msg); // 
}